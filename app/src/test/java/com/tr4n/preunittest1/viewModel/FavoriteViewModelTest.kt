...
class FavoriteViewModelTest {
    ...
    private val fViewModel = FavoriteViewModel(repository)
    ...
    fun `return true if max value of two list is the same`() {
        ...
        val expected = true
        val actual = fViewModel.isTheSameMaxValue(listOf(1, 3), listOf(1, 2, 3))
    ...
    fun `return false if max value of two list is not same`() {
        ...
        every { repository.getMax(listOf(1, 2, 3)) } returns 3
        val expected = false
        val actual = fViewModel.isTheSameMaxValue(listOf(1, 2 ,3), listOf(1, 2))
        Assert.assertEquals(expected, actual)
    }
}